...
def inception_v3_arg_scope(weight_decay=0.00004,  # 设置L2正则的weight_decay
  ...
  # 使用slim.arg_scope后就不需要每次都重复设置参数了，只需要在有修改时设置
    with slim.arg_scope( # 嵌套一个slim.arg_scope对卷积层生成函数slim.conv2d的几个参数赋予默认值
        [slim.conv2d],
        weights_initializer=trunc_normal(stddev), # 权重初始化器 使用截断正态分布 进行初始化
        activation_fn=tf.nn.relu, # 激活函数 使用relu
...
def inception_v3_base(inputs, scope=None):
  ...
  with tf.variable_scope(scope, 'InceptionV3', [inputs]):#scope为包含了函数默认参数的环境
    ...
    with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], # 设置所有模块组的默认参数
      ...
      with tf.variable_scope('Mixed_6a'):
        with tf.variable_scope('Branch_0'):
          branch_0 = slim.conv2d(net, 384, [3, 3], stride=2,
                                 padding='VALID', scope='Conv2d_1a_1x1') # 图片会被压缩 17*17
        with tf.variable_scope('Branch_1'):
          branch_1 = slim.conv2d(net, 64, [1, 1], scope='Conv2d_0a_1x1')
          branch_1 = slim.conv2d(branch_1, 96, [3, 3], scope='Conv2d_0b_3x3')
          branch_1 = slim.conv2d(branch_1, 96, [3, 3], stride=2,
                                 padding='VALID', scope='Conv2d_1a_1x1') # 图片被压缩 17*17
        with tf.variable_scope('Branch_2'):
          branch_2 = slim.max_pool2d(net, [3, 3], stride=2, padding='VALID',
                                     scope='MaxPool_1a_3x3')              # 图片被压缩 17*17
      ...
      with tf.variable_scope('Mixed_7a'):
        with tf.variable_scope('Branch_0'):
          ...
          branch_0 = slim.conv2d(branch_0, 320, [3, 3], stride=2,
                                 padding='VALID', scope='Conv2d_1a_3x3') # 压缩图片 8*8
        with tf.variable_scope('Branch_1'):
          branch_1 = slim.conv2d(net, 192, [1, 1], scope='Conv2d_0a_1x1')
          branch_1 = slim.conv2d(branch_1, 192, [1, 7], scope='Conv2d_0b_1x7')
          branch_1 = slim.conv2d(branch_1, 192, [7, 1], scope='Conv2d_0c_7x1')
      ...
      with tf.variable_scope('Mixed_7b'):
        ...
        with tf.variable_scope('Branch_1'):
          ...
          branch_1 = tf.concat([
              ...
              slim.conv2d(branch_1, 384, [3, 1], scope='Conv2d_0b_3x1')], 3) #8*8*768
        with tf.variable_scope('Branch_2'):
      ...
      with tf.variable_scope('Mixed_7c'):
        ...
        with tf.variable_scope('Branch_3'):
          ...
          branch_3 = slim.conv2d(
              branch_3, 192, [1, 1], scope='Conv2d_0b_1x1')
        net = tf.concat([branch_0, branch_1, branch_2, branch_3], 3) #8*8*2048
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout], # 定义标志默认值
      ...
      with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
        ...
        with tf.variable_scope('AuxLogits'):
          ...
          aux_logits = slim.conv2d(aux_logits, 128, [1, 1], # 卷积。压缩图像尺寸。 5*5*768
                                   scope='Conv2d_1b_1x1')
